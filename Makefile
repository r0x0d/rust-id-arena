...
create-copr-repo:
	copr create --chroot fedora-rawhide-x86_64 \
		--packit-forge-project-allowed https://github.com/r0x0d/$(NAME) \
		$(NAME)
...
create-gh-repo:
	...
	gh repo create rust-id-arena \
		--public \
		--disable-wiki \
		--remote origin \
		--source . \
		--push \
		--description "Upstream rpm repository for id-arena" 
